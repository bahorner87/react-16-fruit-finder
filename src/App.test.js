...
it("renders table that is sorted ascending", async () => {
  const app = mount(<App />);
  await flushRequestsAndUpdate(app);
  expect(app.find("table")).toMatchSnapshot(); //update snapshot - checked and everything matches minus my custom tags
});
...
it("calculates total revenue of all branches", async () => {
  ...
  console.log('[test] - total revenue before', app.find("tfoot td:last-child").text());
  setTimeout(()=> {
    // i'm sorry
...
it("filters the displayed products", async () => {
  ...
  console.log('[test] - filters products("pear") before', app.find("tfoot td:last-child").text());
  setTimeout(()=> {
    // i'm sorry - again
